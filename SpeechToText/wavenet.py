...
def _aconv1d(inputs,
  ...
  with tf.compat.v1.variable_scope(scope, default_name='aconv1d'):
...
def _resnet_block(inputs, num_outputs, kernel_size, rate,
                  ...
                  biases_regularizer=None,
                  scope=None):
  with tf.compat.v1.variable_scope(scope, default_name="block_%d" % rate):
...
def bulid_wavenet(inputs, num_classes, is_training=False, num_hidden_size=128, num_layers=3, rates=[1, 2, 4, 8, 16],
  ...
  outputs = 0
  with tf.compat.v1.variable_scope(scope, default_name='wavenet'):
